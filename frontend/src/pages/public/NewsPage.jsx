...
export default function NewsPage() {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const res = await api.get('/public/articles?type=news')
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="text-slate-500 text-lg">
            តាមដានព្រឹត្តិការណ៍ និងសកម្មភាពផ្សេងៗដែលកើតមានឡើងនៅក្នុង NIEI High School
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <span className="material-icons text-slate-300 text-6xl mb-4">article</span>
            ...
            {articles.map((item) => (
              <Link to={`/article/${item.slug}`} key={item.id}
                ...
                <div className="relative overflow-hidden flex-shrink-0">
                  <img
                    src={item.image_url || 'https://placehold.co/600x400/e2e8f0/64748b?text=News'}
                    alt={item.title}
                    className="h-48 w-full object-cover group-hover:scale-105 transition-transform duration-500"
                  />
                  <div className="absolute top-3 left-3">
                    <span className="bg-white/90 backdrop-blur-sm text-blue-700
